...
const {requireAdmin, check} = require('../auth')
...
router.post('/login', async (req, res) => {
    ...
    if (!check(password)) {
        ...
        const expires = new Date(new Date().getTime() + 86400000 * 365)
        const options =  {expires, domain: "pollofeed.com", httpOnly: true, signed: true}
        res.cookie('adminToken', process.env.ADMIN_TOKEN_PW, options)
